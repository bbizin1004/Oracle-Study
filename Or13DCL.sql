...
/* 하지만 계정에 C##을 붙이는 것은 조금 불편하므로 아래와 같이 session을 
변경하는 명령을 실행한 후 계정을 생성하면 접두어 없이 생성할 수 있다. */
...
/*
2] 생성된 사용자 계정에 권한 혹은 역활 부여
형식] grant 시스템권한1, 권한2, 혹은 역활(Role)
        to 사용자계정
            [with grant 옵션]
*/
grant create session to test_user1;
/*
    create session 권한 부여 후 접속에는 성공했지만, 테이블은 생성되지 않는다. ㅜㅜ 
*/
--테이블 생성 권한 부여
grant create table to test_user1;
/*
    create table 권한 부여 후 테이블 생성 및 desc 명령으로 스키마를 확인할 수 있다.
*/
...
/*
    exit 혹은 quit 명령을 통해 접속을 해제한 후 다시 접속하면 기존의 암호로는 
    접속되지 않는다. 변경한 암호로 접속해야 한다.
*/

/*
4] Role(롤, 역활) 을 통해 여러가지 권한을 동시에 부여하기
...
drop role my_role;
/*
    test_user3은 my_role을 통해 권한을 부여받았으므로 해당 Role을 삭제하면 
    부여받았던 모든 권한이 회수(Revoke) 된다.
    즉, Role 삭제후에는 접속할 수 없다.
*/

/*
5] 권한제거 (회수)
    형식] revoke 권한 혹은 역활 from 사용자계정;
*/
revoke create session from test_user1;

/*
    접속권한 회수 후 접속을 시도하면 비밀번호가 틀린경우에는 '부적합'에러가 발생하고,
...
※ cascade 를 명시하면 사용자계정과 관련된 모든 데이터베이스 스키마가
데이터사전으로 부터 삭제되고 모든 스키마 객체도 물리적으로 삭제된다.
*/

--현재 생성된 사용자 목록을 확인할 수 잇는 데이터사전
...
/*
테이블 스페이스란?
    디스크 공간을 소비하는 테이블과 뷰, 그리고 그 밖의 다른 데이터 베이스 객체들이
    저장되는 장소이다. 예를 들어 오라클을 최초로 설치하면 HR계정의 테이터를 저장하는
    user라는 테이블 스페이스가 자동으로 생성된다.
*/
--테이블 스페이스를 조회할 수 있는 데이터사전
desc dba_tablespaces;
--현재 오라클 시스템에는 총 5개의 테이블스페이스가 생성되어 있다.
select tablespace_name,status, contents from dba_tablespaces;
--우리가 생성한 사용자 계정이 어떤 테이블 스페이스를 사용하는지 조회
select username, default_tablespace from dba_users
where username in upper('test_user2');
-- 사용자에게 테이블 스페이스를 할당한다.(2m은 2MB를 뜻한다.)
alter user test_user2 quota 2m on users;
/*
    즉 test_user2 사용자가 users라는 테이블스페이스에 2MB의 공간을 사용할 수 있도록
    권한을 부여하는것이다. CMD에서 insert 해보면 정상적으로 입력된다.
*/
