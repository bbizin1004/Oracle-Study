...
/*
뷰의 생성
형식] create [or replace] view 뷰의 이름 [(컬럼1,컬럼2,...)] 
    as 
    select * from 테이블명 where조건
    혹은 join 이 있는 select문
    group by 가 추가된 select문 등..
*/
...
--4.데이터사전에서 확인하기(생성시 사용한 쿼리문이 그대로 저장된다)
select * from user_views;

/*
뷰 수정하기
    : 뷰 생성 문장에 or replace 만 추가하면 된다.
    해당 뷰가 존재하면 수정되고, 만약 존재하지 않으면 새롭게 생성된다.
    따라서 최초로 뷰를 생성할때부터 사용해도 무방하다.
*/
...
create or replace view view_emplouees
    (id,fname,jobid,hdate,deptid)
as
    select 
    employee_id, first_name, job_id, hire_date, department_id
    from employees where job_id='ST_CLERK';    
select * from view_emplouees;
/*
    뷰 생성시 기존테이블의 컬럼명을 변경해서 출력하고 싶다면 위와 같이 변경할
    컬럼명을 뷰이름 뒤에 소괄호로 명시해 주면 된다.
*/
...
as
    select 
    employee_id, concat( first_name_||' ', last_name), email, manager_id
...
--뷰로 생성
/*생성시 연산식이 추가되어 논리적으로 컬럼이 추가되는 경우에는 반드시 별칭으로 
컬럼명을 명시해야 한다. 그렇지 않으면 에러가 발생한다.*/
...
select 
    ...
    to_char(hire_date,'yyyy"년"mm"월",dd"일"'),state_province ||'주 의'|| city
from employees
