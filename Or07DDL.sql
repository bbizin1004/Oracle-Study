...
/* Oracle12c 이상부터는 계정 생성전 해당 명령을 실행해야 한다. 
만약 실행하지 않는다면 계정명 앞에 C##을 추가해야 한다.
*/
...
/* 오라클11g 에서는 새로운 계정을 생성한 후  connect , resources 롤(Role)만
부여하면 테이블 생성 및 삽입까지 되지만, 그 이후 버전부터는 테이블스페이스 관련 
오류가 발생한다. 따라서 아래와 같이 테이블 스페이스에 대한 권한도 함께 부여한다.*/
--이 쿼리문은 system 계정에서 실행해야 한다.
grant unlimited tablespace to study;

--레코드 삽입을 위해 study 계정으로 전환 후 실행해야 한다.
