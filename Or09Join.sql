...
/*
1] inner join(내부조인)
-가장 많이 사용하는 조인문으로 테이블간에 연결조건을 모두 만족하는 레코드를 검색할때
사용한다.
-일반적으로 기본키(primary key)와 외래키(foreign key)를 사용하여 join하는 경우가
대부분이다.
-두개의 테이블에 동일한 이름의 컬럼이 존재하면 "테이블명.컬럼명" 형태로
기술해야 한다.
...
--열의 정의가 애매한 경우 컬럼앞에 테이블명을 추가하면 된다.
SELECT
    employee_id,first_name,last_name,email,
    departments.department_id,department_name
from employees inner join departments
    on employees.department_id = departments.department_id;
--as(알리아스)를 통해 테이블에 별칭을 부여하면 간소화 할수 있다.
select
    employee_id,first_name,last_name,email,
...
즉, inner join은 조인한 테이블 양쪽 모두 만족되는 레코드가 인출하게 된다.
*/
...
from locations 
 ...
 --테이블의 별칭을 사용하면 쿼리문을 조금 간단하게 만들수 있다.
 select 
    first_name, last_name,email,
...
/*
2] outer join(외부조인)
outer join 은 inner join 과 달리 두 테이블에 조인조건이 정확히 일치하지 않아도
기준이 되는 테이블에서 레코드를 인출하는 방식이다.
outer join 을 사용할때는 반드시 기준이 되는 테이블을 결정하고 쿼리문을
작성해야 한다.
    -> left(왼쪽 테이블) , right(오른쪽 테이블), full(양쪽테이블)
...
/*실행결과를 보면 내부조인과는 다르게 107개가 인출된다. 부서가 지정되지 않은 사원까지
인출되기 때문인데 , 이 경우 부서쪽에 레코드가 없으므로 null 로 출력된다.*/
...
/*
시나리오] 전체직원의 사원번호, 이름, 부서아이디, 부서명, 지역을 외부조인(left)을 
통해 출력하시오. 단, 오라클 방식으로 작성하시오.
*/
select 
    employee_id,first_name,De.department_id,department_name,city,state_province
...
where E.department_id=D.department_id(+) and to_char(hire_date,'yyyy')='2007';

/*
3] self join(셀프조인)
셀프조인은 하나의 테이블에 있는 컬럼끼리 연결해야 하는 경우 사용한다.
즉 자기자신의 테이블과 조인을 맺는것이다.
셀프조인에서는 별칭이 테이블을 구분하는 구분자의 역활을 하므로 굉장히 중요하다.
...
/*
시나리오] 사원테이블에서 각 사원의 매니저아이디와 매니저 이름을 출력하시오.
...
select
    ...
    concat(empManager.first_name, empManager.last_name)"매니저 이름"
...
/*
    using절에 사용된 참조컬럼의 경우 select절에서 별칭을 붙이면 오히려 에러가 발생한다.
    using절에 사용된 컬럼은 양쪽의 테이블에 동시에 존재하는 컬럼이라는 것을 전재로
    작성되기때문에 굳이 별칭을 사용할 이유가 없기때문이다.
    즉 using은 테이블으 별칭 및 on절을 제거하여 좀 더 심플하게 join쿼리문을 
    작성할 수 있게 해준다.
*/
...
--지역번호 1500을 통해 해당 위치에 있는 부서를 확인한다.(부서번호:50)
select * from departments where location_id=1500;
--위에서 확인한 정보를 토대로 쿼리문 작성
select * from employees where department_id=50
    and to_char(hire_date,'yyyy')='2005';
/*2005년에 입사했고, 50번 부서(shipping)에 근무하는 사원의 정보를 인출해야 한다.*/

--join쿼리문 작성하기.
select
    employee_id, first_name, last_name, 
    trim(to_char(salary,'$990,000')), department_name,country_id,
    country_name
from employees 
