...
/*
현재날짜를 "오늘은 0000년 00월 00일 입니다."와 같은 형식으로 출력하는 
    쿼리문을 작성하시오.
*/
--날짜형식을 인식하지 못해 에러가 발생된다.
select to_char(sysdate, '오늘은 YYYY년MM월DD일 입니다')"과연될까?" from dual;
/*-(하이픈), /(슬러쉬)와 같은 특수기호 이외에는 인식하지 못하므로 
이른 제외한 나머지 문자열은 "으로 묶어줘야 한다. 서식문자를 감싸는게 아니므로
주의해야 한다. */
select to_char(sysdate, '"오늘은 "YYYY"년"MM"월"DD"일 입니다"')"OK"from dual;

/*
시나리오] 사원테이블에서 사원의 입사일을 다음과 같이 출력할 수 있는 쿼리문을 작성하시오
...
select
    to_char(123,'0000'),
    to_char(123,'9999'), trim(to_char(123,'9999')) from dual;
/*
숫자에 세자리마다 컴마 표시하기
:자리수가 확실히 보장된다면 0을 사용하고, 자리수가 다른 부분에서는
9를 사용하여 서식을 지정한다. 대신 공백은 trim() 함수를 통해 제거하면 된다.
*/
select
    12345,
...
/*날짜를 통한 연산을 하고 싶다면 아래와 같이 날짜 변환함수를 사용해야 한다.
날짜에 1을 더하는것은 내일의 날짜를 반환받게 된다.*/
select to_date('2024-01-29') +1 from dual;
/*만약 아래와 같이 날짜 포맷이 년-월-일이 아닌 경우에는 오라클이 인식하지 못해
에러가 발생된다. 이때는 날짜서식을 이용해서 오라클이 인식할 수 있도록 처리해야한다.*/
select to_date('01-29-2021') "에러발생" from dual;
...
*/
/*
to_date('14-10-2021') 이와 같이 기술하면 날짜의 서식을 인식하지 못하므로 아래와 같이
서식문자를 통해 년, 월, 일의 위치를 알려주면 된다.
*/
select 
...
*/ 
/*
방법1: 날짜 형식의 문자열을 substr()로 날짜부분만 잘라낸 후 사용한다.
    주어진 문자열이 년-월-일 형식이므로 그대로 사용할 수 있다.
*/
select to_date(substr('2020-10-14 15:30:21',1,10))from dual;
/*
방법2: 날짜와 시간까지 서식을 활용한다.
*/
select to_date('2020-10-14 15:30:21','yyyy-mm-dd hh24:mi:ss')"시간까지 서식지정"
from dual;
...
*/
--날짜 형식이 알수없으므로 에러발생
select to_char(to_date('2021년01월01일') from daul;


select to_char(to_date('2021년01월01일','yyyy"년"mm"월"dd"일"'),
'day')"요일" from dual;
...
따라서 null값을 가진 컬럼은 처리가 필요하다.*/
select salary+commission_pct from employees;
--null값을 0으로 변환한 후 연산을 진행하면 정상적인 결과를 볼수 있다.
select first_name,commission_pct,salary+nvl(commission_pct,0)from employees;


/*
decode() : Java의 switch문과 비슷하게 특정값에 해당하는 출력문이 있는 경우에 사용한다.
    형식] decode(컬럼명,
                값1,결과1,값2,결과2....
                기본값)
...
/*
case() : Java의 if~else와 비슷한 역활을 하는 함수
    형식] case
        when 조건1 then 값1
        when 조건2 then 값2
        ...
...
6. 사원들의 메니져 사번을 출력하되 매니저가 없는 사원에 대해서는
NULL값 대신 0으로 출력하시오.
