...
/**
primary key : 기본키
-참조 무결성을 유지하기 위한 제약조건으로
-하나의 테이블에 하나의 기본키만 설정할 수 있다.
-기본키로 설정된 컬럼은 중복된 값이나 Null값을 입력할 수 있다.
...
형식1] 인라인방식 컬럼생성시 우측에 제약조건을 기술한다.
    create table 테이블명 (
        컬럼명 자료형(크기) [constrint 제약명] primary key
        );
...
/*
제약조건 및 테이블목록 확인하기
    tab: 현재 계정에 생성된 테애블의 목록을 확인할 수 있다.
    user_cons_columns : 테이블에 지정된 제약조건명과 컬럼명의 간략한 정보를 저장한다.
    user_constraints : 테이블에 지정된 제약조건으 보다 상세한 정보를 저장한다.
※ 이와 같이 제약조건이나 뷰, 프로시저등의 정보를 저장하고 있는 시스템 테이블을 
    "데이터사전"이라고 한다.
*/
select * from tab;
...
insert into tb_primary1 (idx, use_id, user_name)values (3,'white','흰색');
/*pk로 지정된 컬럼에는 null값을 입력할 수 없다. 반드시 중복되지 않는 하나의 값을
입력해야 ㅎ나다. 따라서 에러가 발생한다.*/
insert into tb_primary1 (idx,use_id,user_name)values ('','white','검은색');
--앞에서 입력한 3개의 레코드가 조회된다.
select * from tb_primary1;
/*쿼리문은 문제가 없으나 idx값이 이미 존재하는 2로 변경했으므로 제약조건 위배로
오류가 발생한다.*/
update tb_primary1 set idx=2 where use_id='jongro';
...
/* 테이블을 생성한 후 alter문으로 제약조건을 부여한다. 제약명은 필요에 따라 생략이 
가능하다. */
...
--확인시 휴지통에 들어간것을 볼수 있다.
select * from user_constraints;

--pk는 테이블당 하나만 생성할 수 잇으므로 에러가 발생한다.
...
--입력실패(에러발생)
insert into tb_unique values(6,'이정재','010-4444-4444','배우');
/*
...
-만약 테이블간에 외래키가 설정되어 있다면 자식테이블에 참조값이 존재할 경우 
    부모테이블으 레코드는 삭제할 수 없다.
...
/*자식테이블에서 참조하는 레코드가 있으므로 , 부모테이블의 레코드를 삭제할수 없다.
이 경우 반드시 자식테이블의 레코드를 먼저 삭제한 후 부모테이블의 레코드를 삭제해야한다.*/
delete from tb_primary2 where user_id='jongro'; --오류발생

--자식테이블으 레코드를 먼저 삭제한 후 ..
delete from tb_foreign1 where f_id='jongro';
--부모테이블으 레코드를 삭제하면 정상 처리된다.
...
/*
    ...
    자식테이블에 부모를 참조하는 레코드가 남아있으면 부모테이블의 레코드를 delete
    할 수 없다.
*/
...
create table tb_foreign2 (
    ...
    f_name varchar2(30),
    f_date date,
    /*tb_foreign2 테이블의 f_id 컬럼이 부모테이블인 tb_primary1의 idx컬럼을
    참조하는 외래키를 생성한다.*/
...
[on delete set null]
    :부모레코드 삭제시 자식레토드 값이 null로 변경된다.
    형식] 
        컬럼명 자료형 references 부모테이블 (pk컬럼)
            on delete set null
※ 실무에서 스팸게시물을 남긴 회원과 그 게시글을 일괄적으로 삭제해야 할때 사용할수 있는
옵션이다. 단, 자식테이블의 모든 레코드가 삭제되므로 사용에 주의해야한다.
*/
--user_id를 기본키로 설정한 테이블
create table tb_primary4(
    user_id varchar2(30) primary key,
...
자식쪽까지 모든 레코드가 삭제된다. 만약 해당 옵션을 부여하지 않았다면 레코드는
삭제되지 않고 오류가 발생하게 된다.*/
...
create table tb_not_null (
    ...
    name varchar2(40)/*null 허용 (이와같이 선언한다) */
);
desc tb_not_null;
...
/*오류발생. pk로 지정된 컬럼은 null값을 입력할 수 없다. insert 쿼리에서 컬럼을 
명시하지 않으면 null값이 입력된다. */
