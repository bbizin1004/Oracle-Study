...
시퀀스 & 인덱스
설명: 테이블으 기본키 필드에 순차적인 일련번호를 부여하는 시쿼스와
    검색속도를 향상시킬수 있는 인덱스
*******************/
...
/*
시퀀스(sequence)
-테이블으 컬럼(필드)에 중복되지 않는 순차적인 일련번호를 부여한다.
...
create sequence 시퀀스명
    ...
    [Cycle | NoCycle] -> 최대/최소값에 도달할 경우 처음부터 다시 시작할지
                        여부를 설정(cycle로 지정하면 최대값까지 증가후 다시
                        시작값부터 재 시작됨)
    [Cache | NoCache] -> cache 메모리에 오라클 서버가 시퀀스 값을 할당하는가 
                            여부를 지정

시퀀스 생성시 주의사항
1.start with에 minvalue 보다 작은값을 지정할 수 없다. 즉 start with 값은 
minvalue 와 같거나 커야한다.
...
/*다음 입력할 시퀀스를 반환한다. 실행할때마다 설정한 증가치만큼 증가된 값이 반환된다.*/
select seq_serial_num.nextval from dual;

/*
시퀀스의 nextval을 사용하면 계속 새로운 값을 반환하므로 아래와 같이 insert문에
사용할 수 있다. 또한 같은 문장을 여러번 실행하더라도 문제없이 입력된다.
*/
insert into tb_goods values (seq_serial_num.nextval,'먹태깡1');
/*
    단, 시퀀스의 cycle 옵션에 의해 최대값에 도달하면 다시 처음부터 일련번호가 생성되므로
    무결성 제약조건에 위배된다. 
...
/* 최대값을 지정하지 않는 경우 시퀀스가 표현할 수 잇는 가장 큰값으로 
자동 지정된다. */

/*
인덱스(Index)
-행의 검색속도를 샹상시킬 수 있는 개체
-인덱스는 명시적(create index)과 자동적(primary key, unique)으로 
생성할 수 있다.
-컬럼에 대한 인덱스가 없으면 테이블 전체를 검색한다.
-즉 인덱스는 쿼리의 성능을 양상시키는 것이 그 목적이다.
...
/*데이터 사전에서 확인하기(결과를 보면 PK 혹은 Unique로 지정된 컬럼은 자동으로
인덱스가 생성되어 있는것을 볼수 있다.) */
