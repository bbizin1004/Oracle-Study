...
SQL Developer 에서 주석 사용하기
    블럭단위주석: 자바와 동일하다.
    라인단위주석: -- 실행문자. 하이픈 2개를 연속으로 작성한다.
*/

--select문 : 테이블에 저장된 레코드를 조회하는 SQL문으로 DQL 문에 해당.
...
/*
AS(알리아스): 테디블 혹은 컬럼에 별칭(별명)을 부여할때 사용한다.
        내가 원하는 이름(영문, 한글)으로 변경한 수 출력할 수 있다.
...
/*단, 레코드인 경우 대소문자를 구분한다. 따라서 아래 SQL문을 실행하면 아무런 결과도
인출되지 않는다.*/
select employee_id "사원아이디", first_name "이름" , last_name "성" from employees
where first_name='WILLIAM';
...
/*where절에 2개 이상의 조건이 필요할때 and 혹은 or을 사용할 수 있다.
...
select * from employees where salaty<5000;
--입사일이 04년1월01일 이후인 사원 정보를 추출하시오.
select * from employees where hire_date>='04/01/01';

/*
in 연산자
: or 연산자와 같이 하나의 컬럼에 여러개의 값으로 조건을 걸고 싶을때 사용한다.
=> 급여가 4200, 6400, 8000인 사원의 정보를 추출하시오.
*/
--방법1 : or을 사용한다. 이때 컬럼명을 반복적으로 기술해야 하므로 불편하다.
...
특정 조건으로 select 했을때 하나의 컬럼에서 중복되는 값이 있는경우
...
/*
사원정보 테이블에서 급여가 낮은 순서에서 높은 순설 인출되도록 정력하여 조회하시오.
출력할 컬럼 : 이름, 급여, 이메일, 전화번호
*/
select first_name, salary, email, phone_number from employees order by 
salary asc;
select first_name, salary, email, phone_number from employees order by 
salary; -- asc(오름차순)은 생략할 수 있다.
...
/*
1. 덧셈 연산자를 이용하여 모든 사원에 대해서 $300의 급여인상을 계산한후 이름,급여, 
인상된 급여를 출력하시오.
*/
...
select ename, sal, sal*12+100 as"연봉" from emp order by sal*12+100 desc; 

/*
3. 급여가  2000을 넘는 사원의 이름과 급여를 내림차순으로 정렬하여 출력하시오
*/
select ename , sal from emp where sal>2000 order by sal desc, ename desc;
...
select ename , sal 
from emp where not (sal between 2000 sand 3000) ;
...
select ename ,job ,hiredate 
from emp where hiredate between '81/02/20' and
'81/05/01';
----------------------------------------------------------
/*
1. 7. 부서번호가 20 및 30에 속한 사원의 이름과 부서번호를 출력하되 
