...
/* 시나리오와 문맥상 맞는 쿼리문처럼 보이지만 그룹함수를 단일컬럼에 적용한 잘못된 
쿼리문이므로 에러가 발생한다.*/
...
복수행 연산자: any
    ...
    일치하면 참이되는 연산자. 즉 둘중 하나만 만족하면 해당 레코드를 인출한다.
*/
...
/*
6000 이상이고 동시에 13000보다도 커야하므로 , 결과적으로 13000이상인 레코드만 인출하게
된다. 결과 : 5개
*/

/*
rownum : 테이블에서 레코드를 조회한 순서대로 순번이 부여되는 가상의 컬럼을 말한다.
...
이 경우 rownum은 순서대로 출력된다.( 하지만  이 순서는 오라클이 정하게 되므로
개발자는 알 수 없다.*/
select first_name, rownum from employees;
/*하지만 이름을 기준으로 정렬했을때는 rownum이 섞여져서 이상하게 나오게 된다. */
...
from 절에는 테이블이 들어와야 하지만, 아래의 서브쿼리에서는 사원테이블의 전체 레코드를
대상으로 하되 이름의 오름차순으로 정렬된 상태로 레코드를 가져오게 되므로 테이블을
대체할 수 있게 된다.
*/
