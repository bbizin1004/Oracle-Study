...
사용가능하다.
-트리거는 프로시져의 일종으로 특정테이블에 레코드의 변화가 있을경우
자동으로 실행된다.
-함수는 쿼리문으 일부분으로 사용하기 위해 생성한다. 즉 외부 프로그램에서 호출하는 경우는
거의없다.
-프로시저는 외부 프로그램에서 호출하기 위해 생성한다. 따라서, Java,JSP등에서 간단한
호출로 복잡한 쿼리를 실행할 수 있다.
*/
...
--프로시저 생성시 or replace는 추가하는 것이 좋다.
...
--프로시저 생성시 in파라미터를 설정한다. first_name을 참조하는 참조변수로 선언한다.
...
파라미터는 용도에 따라 in, out을 각각 명시한다. 파라미터 정의시에는 크기는
별도로 명시하지 않는다. */
...
is
    ...
    is절은 비워둔다. 이와같이 변수 선언은 생략할 수 있다. */
begin
    /*in 파라미터는 where절의 조건으로 사용하고, select한 결과값은 into절에서
    out파라미터에 저장한다. 저장된 값은 프로시져 외부로 반환된다. */
    select salary into param_salary
    from employees where first_name= param_name;
end;
/
--호스트 환경에서 바인드변수를 선언한다.
...
/*update 기능이 있는 프로시져 호출을 위해 employees 테이블을 레코드까지
복사한 후 학습을 진행한다.*/

create  table zcopy_employees
as
select * from employees where 1=1;
/* select절에 where을 생략하거나 1=1의 조건을 주면 참이 되므로 모든 레코드를 
함께 복사한다. 만약 스키마(구조)만 복사하고 싶다면 1=0과 같이
부정의 조건을 기술하면 된다. */
...
/*
in 파라미터는 사원번호와 수정할 급여를 입력받는다. out파라미터는 update가 적용된
행의 갯수를 반환한다.
*/
...
begin
    ...
    */
    if SQL%notfound then
        ...
        dbms_output.put_line(SQL%rowcount || '명의 자료가 수정되었씸'); 
        
        --실제 적용된 행의 갯수를 out파라미터를 통해 반환한다.
        rCount :=sql%rowcount;
    end if;
    /*행의 변화가 있는 쿼리를 실행한 경우 반드시 commit을 해야 실제 테이블에
...
--바인드변수 생성 후 프로시저의 반환값(out파라미터)을 저장후 출력한다.
variable r_count number;
execute pcd_update_salary(100,9999,:r_count);
print r_count; --1 행 적용됨
--100번 사원 급여를 확인한다.(결과: 9999로 변경됨)
...
/*
2.함수
-사용자가 PL/SQL문을 사용하여 오라클에서 제공하는 내장함수와 같은 기능을 정의한것이다.
-함수는 in파라미터만 사용할 수 있고, 반드시 반환값의 자료형을 명시해야한다.
-프로시저는 여러개의 결과값을 얻어올 수 있지만, 함수는 반드시 하나의 값을 반환해야 한다.
-함수는 쿼리문의 일부분으로 사용된다.
*/



/* 시나리오] 2개의 정수를 전달받아서 두 정수사이의 모든수를 더해서 결과를
반환하는 함수를 정의하시오.
실행예) 2, 7 -> 2+3+4+5+6+7 = ?? */
...
/*
시나리오] 사원의이름(first_name)을 매개변수로 전달받아서  부서명(department_name)을
반환하는 함수를 작성하시오.
함수명 : func_deptName
*/
...
create or replace function func_deptName (
    param_name varchar2 /*사원으 이름을 인파라미터로 설정*/
    )
return
    /*부서명을 반환해야 하므로 문자형으로 반환타입 지정*/
    varchar2
is
    /* 반환값을 저장할 변수로 부서테이블의 부서명 컬럼을 참조하는 변수로 설정*/
    return_deptname departments.department_name%type;
begin
    /*앞서 작성한 Join쿼리문에서 부서명만 인출할수 있도록 수정한 후
...
create trigger trig_dept_backup
    ...
    after
    /*이벤트 : insert, update, delete 와 같은 쿼리 실행시 발생된다.*/
    INSERT
    --트리거를 적용할 테이블명
    on trigger_dept_original
    /* 행단위 트리거를 정의한다. 즉 하나의 행이 변화할때마다 트리거가
    수행된다. 만약 문장(테이블)단위 트리거로 정의하고 싶다면, 해당 문장을 생략하면
    된다. 이 경우에는 쿼리를 한번 실행하면 트리거도 딱 한번만 실행된다.*/
    for each row
begin
    if Inserting then
...
create or replace trigger  trigPdept_delete
    /* original 테이블으 레코드가 삭제된 후 행단위로 트리거를 적용한다.*/
    after
    delete
    on trigger_dept_original
    for each row
...
--위 조건을 그대로 적용하여 update 한다. (5개의 레코드가 업데이트 된다.)
...
/*
생성2: 오리지날 테이블에 업데이트 이후 테이블(문장) 단위로 발생하는 트리거 생성
*/
create or replace trigger trigger_update_test
    after update on trigger_dept_original
    /*오리지날 테이블으 레코드를 업데이트 한 후 테이블 단위로 트리거가 실행되므로
    무조건 한번만 트리거가 실행된다. 즉 테이블 단위로 트리거를 만들고 싶다면
    아래 옵션을 제거하면 된다.*/
    /* for each row */
begin
    if updating then
        ...
        values(
            ...
            사용할 수 없다. 따라서 아래와 같이 임의의 값을 입력해야 한다.*/
